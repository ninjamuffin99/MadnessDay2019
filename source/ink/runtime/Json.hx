...
class Json
{
	...
	public static function JTokenToRuntimeObject(token:Dynamic):RObject
	{
		...
		if ((Std.isOfType(token, Int) || Std.isOfType(token, Float)))
		{
			// if (Type.typeof(token) == Type.ValueType.TInt
			return Value.Create(token);
		}

		if (Std.isOfType(token, String))
		{
		...
		if (Std.isOfType(token, Array))
		{ // (List<object>
			return JArrayToContainer(token);
		}
		if (Type.typeof(token) == Type.ValueType.TObject)
		{ // token is Dictionary<string, object>  Std.isOfType(token, Dynamic)
			...
			if (propValue != null)
			{ // obj.TryGetValue ("^var", out propValue)
			...
			else if ((propValue = LibUtil.tryGetValueDynamic(obj, "f()")) != null)
			{ // obj.TryGetValue ("f()", out propValue)
				isDivert = true;
				pushesToStack = true;
				divPushType = PushPopType.Function;
			}
			else if ((propValue = LibUtil.tryGetValueDynamic(obj, "->t->")) != null)
			{ // obj.TryGetValue ("->t->", out propValue)
				isDivert = true;
				pushesToStack = true;
				divPushType = PushPopType.Tunnel;
			}
			else if ((propValue = LibUtil.tryGetValueDynamic(obj, "x()")) != null)
			{ // obj.TryGetValue ("x()", out propValue)
				isDivert = true;
				external = true;
				pushesToStack = false;
				divPushType = PushPopType.Function;
			...
			else if ((propValue = LibUtil.tryGetValueDynamic(obj, "CNT?")) != null)
			{ // obj.TryGetValue ("CNT?", out propValue)
			...
			else if ((propValue = LibUtil.tryGetValueDynamic(obj, "temp=")) != null)
			{ // obj.TryGetValue ("temp=", out propValue)
				isVarAss = true;
				isGlobalVar = false;
			}
			if (isVarAss)
	...
	static function ContainerToJArray(container:Container):Array<Dynamic>
	{
		...
		{ // name being set is causing issues?
			...
			if (namedOnlyContent != null)
			{
				...
				for (p in Reflect.fields(terminatingObj))
				{
					...
					var subContainerJArray = LibUtil.as(Reflect.field(terminatingObj, p),
						Array); // namedContentObj.Value; // var subContainerJArray = namedContentObj.Value as List<object>;
