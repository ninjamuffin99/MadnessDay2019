...
class Story extends RObject
{
	...
	function VariableStateDidChangeEvent(variableName:String, newValueObj:RObject):Void
	{
		...
		{ // _variableObservers.TryGetValue (variableName, out observers)
			...
			if (!(Std.isOfType(newValueObj, Value)))
			{
	...
	function Step()
	{
		...
		if (Std.isOfType(currentContentObj, Container))
		{
			shouldAddToStream = false;
		}
	...
	{
		var truthy = false;
		if (Std.isOfType(obj, Value))
		{
			// var val:Value = cast obj; //(Value)
			// var val:Dynamic = obj;
			var val = LibUtil.as(obj, Value);

			if (Std.isOfType(val, DivertTargetValue))
			{
	...
	function PerformLogicAndFlowControl(contentObj:RObject):Bool
	{
		...
		// Divert
		if (Std.isOfType(contentObj, Divert))
		{
			...
			if (currentDivert.hasVariableTarget)
			{
				...
				if (!(Std.isOfType(varContents, DivertTargetValue)))
				{
		...
		else if (Std.isOfType(contentObj, ControlCommand))
		{
			...
			switch (evalCommand.commandType)
			{
				...
				case ControlCommand.CommandType.EvalOutput:
					...
					if (state.evaluationStack.length > 0)
					{
						...
						if (!(Std.isOfType(output, VoidObj)))
						{
				...
				case ControlCommand.CommandType.EndString:
					...
					while (i >= 0)
					{
						...
						if (Std.isOfType(obj, StringValue))
				...
				case ControlCommand.CommandType.TurnsSince:
					...
					if (!(Std.isOfType(target, DivertTargetValue)))
					{
						var extraNote = "";
						if (Std.isOfType(target, IntValue))
		...
		// Variable assignment
		else if (Std.isOfType(contentObj, VariableAssignment))
		{
		...
		// Variable reference
		else if (Std.isOfType(contentObj, VariableReference))
		{
		...
		else if (Std.isOfType(contentObj, NativeFunctionCall))
		{
	...
	function VisitChangedContainersDueToDivert():Void
	{
		...
		{
			var prevAncestor:Container = Std.isOfType(previousContentObject,
	...
	**/
	...
	public function HasFunction(functionName:String):Bool
	{
		try
		{
			return Std.isOfType(ContentAtPath(Path.createFromString(functionName)), Container);
		}
