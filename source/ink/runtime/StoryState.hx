...
class StoryState
{
	...
	**/
	...
	function set_jsonToken(value:Dynamic):Dynamic
	{
		...
		{ // !jObject.TryGetValue("inkSaveVersion", out jSaveVersion)
		...
		{ // jObject.TryGetValue("currentDivertTarget", out currentDivertTargetPath)
	...
	function get_currentGlueIndex():Int
	{
		...
		while (i >= 0)
		{
			...
			else if (Std.isOfType(c, ControlCommand)) // e.g. BeginString
				break;
			i--; // continuing...
		}
		return -1;
	...
	function get_outputStreamEndsInNewline():Bool
	{
		if (_outputStream.length > 0)
		{
			...
			while (i >= 0)
			{
				...
				if (Std.isOfType(obj, ControlCommand)) // e.g. BeginString
					break;
	...
	function get_outputStreamContainsContent():Bool
	{
		...
		{
			if (Std.isOfType(content, StringValue))
				return true;
		}
		return false;
	}
	...
	public function PopEvaluationStack1(numberOfObjects:Int):Array<RObject> // List<Object>
	{
		...
		var popped = evaluationStack.slice(evaluationStack.length - numberOfObjects,
			evaluationStack.length - numberOfObjects + numberOfObjects); // evaluationStack.GetRange(evaluationStack.length - numberOfObjects, numberOfObjects);
		evaluationStack.splice(evaluationStack.length - numberOfObjects,
			numberOfObjects); // evaluationStack.RemoveRange (evaluationStack.length - numberOfObjects, numberOfObjects);
		return popped;
	}
