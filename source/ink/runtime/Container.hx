...
class Container extends RObject implements INamedContent
{
	...
	{
		Assert.bool(Std.isOfType(namedContentObj, RObject), "Can only add Runtime.Objects to a Runtime.Container");
	...
	public function BuildStringOfHierarchy(sb:StringBuf, indentation:Int, pointedObj:RObject)
	{
		...
		{ // int i=0; i<content.Count; ++i
			...
			//
			if (!(Std.isOfType(obj, Container)) && obj == pointedObj)
		...
		{ // onlyNamed.length > 0
			...
			for (k in onlyNamed.keys())
			{
				...
				Assert.bool(Std.isOfType(objV, Container), "Can only print out named Containers");
