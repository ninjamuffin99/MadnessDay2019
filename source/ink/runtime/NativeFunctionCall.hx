...
class NativeFunctionCall extends RObject
{
	...
	static function GenerateNativeFunctionsIfNecessary():Void
	{
		if (_nativeFunctions == null)
		{
			...
			AddIntBinaryOp(Add, function(x, y)
			{
				return x + y;
			});
			AddIntBinaryOp(Subtract, function(x, y)
			{
				return x - y;
			});
			AddIntBinaryOp(Multiply, function(x, y)
			{
				return x * y;
			});
			AddIntBinaryOp(Divide, function(x, y)
			{
				return Std.int(x / y);
			});
			AddIntBinaryOp(Mod, function(x, y)
			{
				return x % y;
			});
			AddIntUnaryOp(Negate, function(x)
			{
				return -x;
			});

			AddIntBinaryOp(Equal, function(x, y)
			{
				return x == y ? 1 : 0;
			});
			AddIntBinaryOp(Greater, function(x, y)
			{
				return x > y ? 1 : 0;
			});
			AddIntBinaryOp(Less, function(x, y)
			{
				return x < y ? 1 : 0;
			});
			AddIntBinaryOp(GreaterThanOrEquals, function(x, y)
			{
				return x >= y ? 1 : 0;
			});
			AddIntBinaryOp(LessThanOrEquals, function(x, y)
			{
				return x <= y ? 1 : 0;
			});
			AddIntBinaryOp(NotEquals, function(x, y)
			{
				return x != y ? 1 : 0;
			});
			AddIntUnaryOp(Not, function(x)
			{
				return x == 0 ? 1 : 0;
			});

			AddIntBinaryOp(And, function(x, y)
			{
				return x != 0 && y != 0 ? 1 : 0;
			});
			AddIntBinaryOp(Or, function(x, y)
			{
				return x != 0 || y != 0 ? 1 : 0;
			});

			AddIntBinaryOp(Max, function(x, y)
			{
				return LibUtil.maxI_(x, y);
			});
			AddIntBinaryOp(Min, function(x, y)
			{
				return LibUtil.minI_(x, y);
			});

			// Float operations
			AddFloatBinaryOp(Add, function(x, y)
			{
				return x + y;
			});
			AddFloatBinaryOp(Subtract, function(x, y)
			{
				return x - y;
			});
			AddFloatBinaryOp(Multiply, function(x, y)
			{
				return x * y;
			});
			AddFloatBinaryOp(Divide, function(x, y)
			{
				return x / y;
			});
			AddFloatBinaryOp(Mod, function(x, y)
			{
				return x % y;
			}); // TODO: Is this the operation we want for floats?
			AddFloatUnaryOp(Negate, function(x)
			{
				return -x;
			});

			AddFloatBinaryOp(Equal, function(x, y)
			{
				return x == y ? 1 : 0;
			});
			AddFloatBinaryOp(Greater, function(x, y)
			{
				return x > y ? 1 : 0;
			});
			AddFloatBinaryOp(Less, function(x, y)
			{
				return x < y ? 1 : 0;
			});
			AddFloatBinaryOp(GreaterThanOrEquals, function(x, y)
			{
				return x >= y ? 1 : 0;
			});
			AddFloatBinaryOp(LessThanOrEquals, function(x, y)
			{
				return x <= y ? 1 : 0;
			});
			AddFloatBinaryOp(NotEquals, function(x, y)
			{
				return x != y ? 1 : 0;
			});
			AddFloatUnaryOp(Not, function(x)
			{
				return x == 0.0 ? 1 : 0;
			});

			AddFloatBinaryOp(And, function(x, y)
			{
				return x != 0.0 && y != 0.0 ? 1 : 0;
			});
			AddFloatBinaryOp(Or, function(x, y)
			{
				return x != 0.0 || y != 0.0 ? 1 : 0;
			});

			AddFloatBinaryOp(Max, function(x, y)
			{
				return Math.max(x, y);
			});
			AddFloatBinaryOp(Min, function(x, y)
			{
				return Math.min(x, y);
			});

			// String operations
			AddStringBinaryOpConcat(Add, function(x, y)
			{
				return x + y;
			}); // concat
			AddStringBinaryOp(Equal, function(x, y)
			{
				return x == y ? 1 : 0;
			});
			...
			//
			var divertTargetsEqual:BinaryOpPath = function(d1, d2)
			{ // BinaryOp<Path>
	...
	static function AddOpToNativeFunc(name:String, args:Int, valType:ValueType, op:Dynamic):Void
	{
		...
		if (!(nativeFunc != null))
		{ // !_nativeFunctions.TryGetValue (name, out nativeFunc)
