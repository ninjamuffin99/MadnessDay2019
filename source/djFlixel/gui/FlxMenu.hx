...
class FlxMenu extends FlxGroup
{
	...
	public function new(X:Float, Y:Float, WIDTH:Int = 0, SlotsTotal:Int = 6)
	{
		...
		animQ.queue_action = function(fn:Void->Void)
		{
			fn();
		};
		_popupCloseFn = null;
	...
	public function applyMenuStyle(stMenu:Dynamic, ?stHeader:Dynamic)
	{
		...
		if (stHeader != null)
			DataTool.copyFieldsC(stHeader, styleHeader);
	} //---------------------------------------------------;
	...
	public function item_updateData(pageSID:String, SID:String, param:Dynamic)
	{
		...
		if (a != null)
		{ // If it actually found the menu somewhere
	...
	{
		if (Std.isOfType(Page, String)) // It's an SID
		{
	...
	public function close(rememberState:Bool = false)
	{
		...
		if (rememberState && currentPage != null)
		{
			...
			if (d1 != null)
				currentPage.custom._cursorLastUID = d1.UID;
		}
	...
	function poolGetOrNew(P:PageData):VListMenu
	{
		...
		var m = new VListMenu(x, y, P.custom.width != null ? P.custom.width : width, P.custom.slots != null ? P.custom.slots : slotsTotal);
	...
	function _sub_confirmFullPage(o:MItemData)
	{
		...
		if (!pages.exists(questionPageID))
		{
			...
			if (o.data.conf_p_style != null)
				p.custom.styleMenu = o.data.conf_p_style;
	...
	private function popup_YesNo(X:Float, Y:Float, qcallback:Bool->Void, ?question:String, ?yesno:Array<String>):Void
	{
		...
		if (yesno.length != 2)
		{
			trace("Error: yesno must have exactly 2 strings");
			return;
		}
		#end

		if (currentMenu != null)
			currentMenu.setInputFocus(false);
		...
		s.fontSize = Std.int(s.fontSize / 2);
		if (s.fontSize < 8)
			s.fontSize = 8; // MIN
		...
		_popupCloseFn = function()
		{
			...
			if (currentMenu != null)
				currentMenu.setInputFocus(true);
		}
		...
		if (item != null)
		{
			...
			switch (status)
			{
				case "fire":
					_q_callback("tick_fire");
				case "change":
					_q_callback("tick_change");
				case "invalid":
					_q_callback("tick_error");
			}
		}
